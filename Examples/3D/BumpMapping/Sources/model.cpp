...
public:
	...
	void Load(CL_GraphicContext &gc, const char *filename, bool generate_texture_coords);
...
Model::Model(CL_GraphicContext &gc, const char *filename, bool generate_texture_coords): impl(new Model_Impl())
{
	impl->Load(gc, filename, generate_texture_coords);
}
...
void Model_Impl::Load(CL_GraphicContext &gc, const char *filename, bool generate_texture_coords)
{
...
void Model_Impl::Draw(CL_GraphicContext &gc, GraphicStore *gs, const CL_Mat4f &modelview_matrix)
{
	...
	prim_array.set_attributes(0, vbo_positions, 3, cl_type_float);
	prim_array.set_attributes(1, vbo_normals, 3, cl_type_float);

	if (vbo_texcoords.is_null())
		throw CL_Exception("Shader not supported");

	prim_array.set_attributes(2, vbo_texcoords, 2, cl_type_float);

	gc.set_texture(0, gs->texture_brick);
	gc.set_texture(1, gs->normal_map);
	gs->shader_bumpmap.SetMaterial(material_shininess, material_emission, material_ambient, material_specular);

	gs->shader_bumpmap.Use(gc);
